...
public class RaycastWeapon : MonoBehaviour
{
    ...
    public ParticleSystem buildingHitEffect;
    ...
    private void Awake()
    {
        ...
        // Добавляем AudioSource для звука выстрела
    ...
    void RaycastSegment(Vector3 start, Vector3 end, Bullet bullet)
    {
        ...
        {
            // Проверяем, здание ли это
            bool isBuilding = hitInfo.collider.gameObject.layer == LayerMask.NameToLayer("Building");

            if (isBuilding && buildingHitEffect != null)
            {
                // ЭФФЕКТ ДЛЯ ЗДАНИЯ
                ParticleSystem effect = Instantiate(buildingHitEffect, hitInfo.point, Quaternion.LookRotation(hitInfo.normal));
        ...
        if (debug)
        {
            Debug.DrawLine(start, end, Color.green, 1.0f);
        }
    }
